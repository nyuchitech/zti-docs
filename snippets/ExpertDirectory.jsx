...
export const ExpertDirectory = ({ showFilters = true, category: initialCategory = null }) => {
  ...
  const fetchProfessionals = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      // Cross-schema joins (hospitality.professional → identity.person) are not
      // supported via Accept-Profile — select only columns on professional itself.
      const url =
        `${SUPABASE_URL}/rest/v1/professional` +
        `?select=id,name,occupation_type,specialisations,years_experience,license_type,` +
        `services_offered,booking_url,verification_tier,aggregate_rating,review_count,featured,` +
        `image,bio,email,telephone` +
          ...
          {filtered.map((p) => {
            ...
            return (
              ...
              >
                ...
                {p.bio && (
                  <p className="mt-3 text-sm text-gray-600 dark:text-gray-300 line-clamp-2">{p.bio}</p>
                )}
              </div>
            );
          })}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-5 flex justify-between items-start">
              <div className="flex items-center gap-4">
                {selected.image ? (
                  <img src={selected.image} alt={selected.name} className="w-16 h-16 rounded-full object-cover" />
                ) : (
                  <div className="w-16 h-16 rounded-full bg-primary-100 dark:bg-primary-900 flex items-center justify-center">
                    <span className="text-2xl font-bold text-primary-600 dark:text-primary-400">{(selected.name || 'U').charAt(0)}</span>
                  </div>
                )}
                <div>
            ...
            <div className="p-6 space-y-5">
              ...
              {selected.bio && (
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-300 leading-relaxed">{selected.bio}</p>
                </div>
              )}
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-5">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {selected.telephone && (
                    <a href={`https://wa.me/${selected.telephone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium">
